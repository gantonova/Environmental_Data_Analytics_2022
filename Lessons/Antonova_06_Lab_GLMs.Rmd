...
NOTE:Two sample t-test is very similar to one-way ANOVA.The difference is that for the t-test, you can only have two levels for the categorical variable. 

```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018])
shapiro.test(EPAair$Ozone[EPAair$Year == 2019])
#use ??shapiro.test for explanation. shapiro test tests the null that the data is normally distributed  
...
# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year, var.equal=TRUE)
#using the var.equal true to mimic the result of the ANOVA
O3.twosample
...
```{r}
#Exercise 2:
densityOfOzone <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  geom_density(alpha=0.5)+
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
  labs(x="Ozone AQI value", y = "Density", fill ="")

print(densityOfOzone)

#use fill so that the whole box will be colored, if you use color only the line defining the plot will be colored
...
PM25fromOzone.regression <- lm(data = EPAair, PM2.5 ~ Ozone)
#order matters - the first one is the dependent variable and the second is the independent

summary(PM25fromOzone.regression)
...
library(corrplot)
#create the subset
EPAairCorr <- cor(EPAair)
corrplot(EPAairCorr, method = "ellipse")
corrplot.mixed(EPAair, upper = "ellipse")

PM25fromOzoneandMonth.regression <- lm(data = EPAair, PM2.5 ~ Month + Ozone )
#order matters - the first one is the dependent variable and the second is the independent

summary(PM25fromOzoneandMonth.regression)

PM25fromOzoneandCounty.regression <- lm(data = EPAair, PM2.5 ~ COUNTY + Ozone )
#order matters - the first one is the dependent variable and the second is the independent

summary(PM25fromOzoneandCounty.regression)
...
TPAIC <- lm(data = EPAair, PM2.5 ~ Ozone + COUNTY + 
              Date + Site.Name + meanLat +meanLng +Month + Year)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
TPmodel <- lm(data = EPAair, PM2.5 ~ Ozone + COUNTY + 
              Date + Site.Name + meanLat +meanLng +Month + Year)
summary(TPmodel)
