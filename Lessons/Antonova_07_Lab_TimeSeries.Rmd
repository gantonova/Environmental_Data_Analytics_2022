...
#inner join helps check for NAs - not sure how 
...
#if we had NAs there would be a column indicating that when using the summary function

is.na(BRA_complete$Rainfall)
is.na(BRA_complete$Temperature_C)
...
BRA_complete_Rain_ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)

BRA_complete_Temp_ts <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)
...
BRA_complete_Rain_Decomposed <- stl(BRA_complete_Rain_ts, s.window = "periodic")
BRA_complete_Temp_Decomposed <- stl(BRA_complete_Temp_ts, s.window = "periodic")


# Visualize the decomposed series. 
plot(BRA_complete_Rain_Decomposed)
plot(BRA_complete_Temp_Decomposed)


# We can extract the components and turn them into data frames
BRA_complete_Temp_Components <- as.data.frame(BRA_complete_Temp_Decomposed$time.series[,1:3])

BRA_complete_Rain_Components <- as.data.frame(BRA_complete_Rain_Decomposed$time.series[,1:3])
...
BRA_complete_Rain_Components <- mutate(BRA_complete_Rain_Components)
