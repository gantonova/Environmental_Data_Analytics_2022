...
```{r}
high <- c(42, 45, 48, 35, 43, 52, 51, 60, 66, 59)

low <- c(20, 27, 27, 15, 26, 25, 33, 49, 48, 27)
class(low)
print(low)
```
...
return(convert)}
highCelsius <- convert(high)
lowCelsius <- convert(low)
print(lowCelsius)
print(highCelsius)
```
...
temperature<- as.data.frame(low)
temperature <- as.data.frame(high)
temperature <- as.data.frame(highCelsius)
temperature <- as.data.frame(lowCelsius)
df_temperatures_C_and_F <- cbind(low, high, highCelsius, lowCelsius)
class(df_temperatures_C_and_F)
...
```{r}
summary(df_temperatures_C_and_F)
sd(df_temperatures_C_and_F$low)
```
