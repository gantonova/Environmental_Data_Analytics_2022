...
NTL.phys.data.Tuesday.90to99 <- filter(NTL.phys.data, year4 < 2000 & year4 > 1989 & lakename == "Tuesday Lake")
...
library(lubridate)
NTL.phys.data.Tuesday.90to99.pipe <- 
  NTL.phys.data %>%
  filter(year4 < 2000)%>%
  filter(year4 > 1989)%>%
...
NTL.phys.data.Tuesday.90to99.mean <-
NTL.phys.data.Tuesday.90to99.pipe %>%
  filter(depth==0)%>%
  filter(temperature_C !="NA")%>%
  summarise(mean = mean(temperature_C))
class()
  
#or

NTL.phys.data.Tuesday.90to99.mean2 <-
NTL.phys.data.Tuesday.90to99.pipe %>%
  filter(depth==0)%>%
