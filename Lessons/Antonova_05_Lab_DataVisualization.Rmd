...
# 1. 
# Plot surface temperatures by day of  year. (scatterplot)
# Color your points by year, and facet by lake in two rows.
# Change the ylab name (function is ylab(new name))


#filter
PeterPaul_surface_only <- filter(PeterPaul.chem.nutrients, depth==0)
#can also use the subset function instead 

#color
color_scatter_plot <- ggplot(PeterPaul_surface_only, aes(x = daynum, y = temperature_C, color = year4)) + 
  geom_point()
print(color_scatter_plot)
#can also put the color in a new aes next to geom_point 
#when you color by a category, the "color" needs to go inside the aes

#facet
color_scatter_faceted <- ggplot(PeterPaul_surface_only, aes(x = daynum, y = temperature_C, color = year4)) + 
  geom_point() +
  facet_wrap(vars(lakename), nrow=2)

print(color_scatter_faceted)
...
temperature_by_date <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C)) +
...
print(temperature_by_date)

temperature_by_date_reverse <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C)) +
  geom_point(aes(color=depth), size=0.5)+
  ylab("temp in C") +
  scale_color_viridis(option = "magma", direction = 1) +
  scale_x_date(
...
print(temperature_by_date_reverse)
